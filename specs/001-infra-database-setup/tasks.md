...
- [ ] T001 Create infra/ directory structure per plan.md (`infra/migrations/`, `scripts/`)
- [ ] T002 Create `.gitignore` with PostgreSQL data volume exclusions
- [ ] T003 Add `AGENTS.md` reference to tasks.md (if not already present)
...
- [ ] T004 Create `infra/docker-compose.yml` with both DB services, networks, volumes per contracts/docker-compose.md
- [ ] T005 Create `infra/.env.example` with all env vars documented per contracts/env-vars.md
- [ ] T006 Create `scripts/dev.sh` single-command startup wrapper with health-check polling
...
- [ ] T007 [P] [US1] Create `infra/migrations/001-platform-db-init.sql` — create schemas, enable PostGIS per data-model.md
- [ ] T008 [P] [US1] Create `infra/migrations/004-analytics-db-init.sql` — create raw_events table with append-only rules per data-model.md
- [ ] T009 [US1] Create `scripts/init-dbs.sh` — run platform_db migrations (001) then analytics_db migrations (004) with idempotency
- [ ] T010 [US1] Wire migration script into `scripts/dev.sh` as a post-health-check step
- [ ] T011 [US1] Update `scripts/dev.sh` to validate connectivity (pg_isready + schema inspection)
- [ ] T012 [US1] Test restart persistence: `docker compose down && docker compose up` — data survives
...
- [ ] T013 [US2] Create `infra/migrations/002-inventory-schema.sql` — partner, station, charger tables + all indexes from data-model.md
- [ ] T014 [US2] Create `infra/migrations/003-gis-schema.sql` — osm_region, osm_road tables + spatial indexes from data-model.md
- [ ] T015 [US2] Append 002 and 003 into `scripts/init-dbs.sh` migration sequence
- [ ] T016 [US2] Validate idempotency: re-run init-dbs.sh — no errors, no duplicate objects
- [ ] T017 [US2] Verify spatial index via `EXPLAIN SELECT ... ST_DWithin(location, ...)` — index scan (not seq scan)
- [ ] T018 [US2] Verify append-only rules on analytics_db: attempt UPDATE/DELETE on raw_events — both rejected
...
- [ ] T019 [US3] Create `infra/migrations/005-seed-data.sql` with:
  - [ ] 2+ partners (e.g., TotalEnergies TN, STEG)
  - [ ] 3+ stations with real Tunisia coordinates (Tunis, Sfax, Sousse)
  - [ ] 5+ chargers across CCS2, CHAdeMO, Type2 types
- [ ] T020 [US3] Append 005 to end of `scripts/init-dbs.sh` migration sequence
- [ ] T021 [US3] Validate seed data: nearby search returning results ordered by distance with correct values
- [ ] T022 [US3] Validate idempotency: re-run seed migration — no duplicate rows (use IF NOT EXISTS or idempotent insert pattern)
...
- [ ] T023 [P] Run full `scripts/dev.sh` from clean state — measure time to healthy (< 5 min)
- [ ] T024 [P] Test port conflict scenario: start service when port 5432 is occupied — graceful error message
- [ ] T025 [P] Test container registry unavailable — graceful error message
- [ ] T026 [P] Quickstart verification: follow quickstart.md instructions end-to-end, fix any gaps
- [ ] T027 [P] Run schema inspection checklist from checklists/requirements.md — all items pass
- [ ] T028 [P] Verify all SC metrics from spec.md: SC-001 through SC-007
