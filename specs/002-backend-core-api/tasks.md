...
- [X] T001 Create directory structure for driver-service: `source/services/driver-service/src/lib/handlers/`, `src/lib/models/`, `src/lib/services/`, `src/lib/repositories/`, `tests/integration/`, `tests/fixtures/`
- [X] T002 [P] Create `source/services/driver-service/Cargo.toml` with dependencies: tokio 1.52, axum 0.8, sea-orm 2.x (features: sqlx-postgres, runtime-tokio-native-tls, macros), serde 1.0 (derive), serde_json 1.0, tracing 0.1, tracing-subscriber 0.3 (env-filter), thiserror 2.0, dotenvy 0.15, utoipa 5.5 (axum), utoipa-swagger-ui 9.0 (axum), tower 0.5; dev-deps: mockall 0.14, tower 0.5
- [X] T003 [P] Create `source/services/driver-service/rustfmt.toml` with max_width=120 and configure `.cargo/config.toml` for nightly clippy if needed
...
- [X] T004 [P] Create Station domain model in `source/services/driver-service/src/models/station.rs` with fields: id (String), name (String), status (String), latitude (Option<f64>), longitude (Option<f64>), distance (f64). Derive Serialize, Deserialize, Debug, Clone. utoipa::ToSchema deferred to Phase 6. Create models/mod.rs re-export.
- [X] T005 [P] Create error types in `source/services/driver-service/src/error.rs`: DomainError enum with variants NotFound(String), BadRequest(String), ServiceUnavailable(String), Internal(String). Derive thiserror::Error. Implement IntoResponse for axum response mapping (404/400/503/500). Add error response JSON shape per contracts/api.md.
- [X] T006 [P] Create Config struct in `source/services/driver-service/src/config.rs` with fields: database_url, host, port, db_pool_size, db_connect_retries, db_retry_base_delay_ms, rust_log. Implement Config::from_env() using dotenvy + std::env with defaults per research.md.
- [X] T007 [P] Create repository trait in `source/services/driver-service/src/repositories/mod.rs` with StationRepository trait (Send + Sync) defining methods: find_all, find_by_id, find_nearby. Add #[cfg_attr(test, mockall::automock)] for mock generation. Create StationRepositoryImpl struct holding PgPool. Create repositories/mod.rs re-export.
- [X] T008 Implement `source/services/driver-service/src/main.rs`: Config::from_env(), tracing_subscriber init with EnvFilter, DB connection pool with exponential backoff retry (default 3 retries, 1s base), crash on exhaustion. Router via driver_service::build_router().
- [X] T009 [P] Create test fixtures in `source/services/driver-service/tests/fixtures/stations.sql` with the 4 seeded stations from Sprint 0 OSM import (STA-00001 through STA-00004) matching inventory.station schema.
- [X] T010 [US4] Implement health handler in `source/services/driver-service/src/handlers/health.rs`: GET /api/v1/health returns 200 with {"status":"ok","database":"connected"} when DB pool is healthy, 200 with {"status":"error","database":"disconnected"} when DB is unreachable. Create handlers/mod.rs with module declaration.
- [X] T011 [US4] Wire health route into Router in lib.rs and create integration test in `source/services/driver-service/tests/health_test.rs` using tower::oneshot to verify 200 + JSON body shape + DB connectivity field.
...
- [X] T012 [US1] Implement `StationRepository::find_all` in `source/services/driver-service/src/repositories/mod.rs` using sqlx raw SQL to query inventory.station. Return Vec<Station>. Map NULL name to empty string. Filter out stations with NULL latitude/longitude.
- [X] T013 [US1] Implement StationService in `source/services/driver-service/src/services/station_service.rs`: struct wrapping generic StationRepository trait (for testability via mockall). Method list_stations() calling repo.find_all(). Create services/mod.rs re-export. Add unit test using MockStationRepository.
- [X] T014 [US1] Implement list handler in `source/services/driver-service/src/handlers/stations.rs`: GET /api/v1/stations handler calling service.list_stations(). Return 200 with JSON array. Return 500 on DB error. Update handlers/mod.rs with stations module.
- [X] T015 [US1] Wire list route into Router in lib.rs. Create integration test in `source/services/driver-service/tests/stations_test.rs` using tower::oneshot: verify stations returned with correct field names, skip gracefully when no test DB configured.
...
- [X] T016 [US2] Implement `StationRepository::find_by_id` in `source/services/driver-service/src/repositories/mod.rs` using sqlx raw SQL on inventory.station. Return Option<Station> (None → 404). Map NULL name to empty string.
- [X] T017 [US2] Add get_station method to StationService in `source/services/driver-service/src/services/station_service.rs`: calls repo.find_by_id(id), maps None to DomainError::NotFound. Add unit test for found/not-found paths using MockStationRepository.
- [X] T018 [US2] Add get-by-id handler in `source/services/driver-service/src/handlers/stations.rs`: GET /api/v1/stations/{id} handler with axum Path param. Return 200 with station JSON, 404 for not found, 400 for invalid ID format.
- [X] T019 [US2] Wire detail route into Router in lib.rs. Add integration tests in `source/services/driver-service/tests/stations_test.rs`: verify existing ID returns correct station, non-existent ID returns 404 JSON body, ID format validation.
...
- [X] T020 [US3] Implement `StationRepository::find_nearby` in `source/services/driver-service/src/repositories/mod.rs` using sqlx raw SQL executing PostGIS query: `SELECT ... FROM inventory.station WHERE status = 'active' AND ST_DWithin(location, ST_SetSRID(ST_MakePoint($lng, $lat), 4326)::geography, $radius) ORDER BY ST_Distance(...) ASC`. Return Vec<Station> with computed distance field. Filter NULL coordinates.
- [X] T021 [US3] Add find_nearby method to StationService in `source/services/driver-service/src/services/station_service.rs`: validate lat (-90 to 90), lng (-180 to 180), radius (>0, default 5000). Call repo.find_nearby. Add unit tests for validation rules using MockStationRepository.
- [X] T022 [US3] Implement nearby handler in `source/services/driver-service/src/handlers/nearby.rs`: GET /api/v1/stations/nearby with axum Query params (lat, lng, optional radius). Extract and forward to service. Return 200 with array (empty if none found), 400 for invalid params. Create handlers/nearby.rs and update handlers/mod.rs.
- [X] T023 [US3] Wire nearby route into Router in lib.rs. Create integration test in `source/services/driver-service/tests/nearby_test.rs`: verify proximity ordering by distance ascending, empty result for far-away query, 400 for missing lat/lng, 400 for out-of-range values, response under 200ms.
