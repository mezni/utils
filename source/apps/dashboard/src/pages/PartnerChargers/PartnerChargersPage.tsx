...
export function PartnerChargersPage() {
  const { selectedPartnerId } = useRole();
  const [data, setData] = useState<ChargerRow[]>([]);
  const [ownStations, setOwnStations] = useState<Station[]>([]);
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const stations = await list<Station>('stations', { partner_id: selectedPartnerId });
      setOwnStations(stations);
      ...
      const chargers = await list<Charger>('chargers?' + qs);
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="text-base font-semibold text-main">My Chargers</h2>
          {ownStations.length > 1 && (
            <select value={filter} onChange={(e) => setFilter(e.target.value)} className="rounded border border-default px-2 py-1 text-sm">
              ...
              {ownStations.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          )}
        </div>
