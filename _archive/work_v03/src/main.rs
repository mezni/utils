...
async fn main() -> Result<()> {
    ...
    for input in inputs {
        match process_syslog(input, &syslog_config, &mut timestamps, &mut session_ids) {
            Ok(()) => {
    ...
    // Perform filtering (e.g., session IDs >= 8000)
    //let filtered_df = dataframe.filter(col("session_id").gt_eq(lit("8000")))?;
