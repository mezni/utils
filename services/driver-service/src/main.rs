...
async fn stations_handler(auth: AuthCtx) -> impl IntoResponse {
    match auth.0 {
        Some(ref user) => {
            Json(serde_json::json!({ "stations": [], "user_id": user.user_id, "roles": user.roles }))
        }
        None => (
            StatusCode::UNAUTHORIZED,
...
async fn main() {
    ...
    let app = Router::new()
        .route("/api/v1/driver/stations", get(stations_handler))
