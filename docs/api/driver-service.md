...
|-------|------|----------|-------------|
| id | string | Yes | Station identifier (STA-xxx) |
| name | string | Yes | Station name |
| latitude | number | Yes | Station latitude |
| longitude | number | Yes | Station longitude |
| status | string | Yes | Station status (active) |
| power_kw | number | No | Maximum power capacity in kW |
| connector_types | string[] | No | Supported connector types |

---

### 1.2 Nearby Stations (CORE MVP FEATURE)
...
| latitude | number | Yes | Station latitude |
| longitude | number | Yes | Station longitude |
| distance | number | Yes | Distance from user in meters |
| status | string | Yes | Station status (active) |

**Behavior Rules:**
- **Sorted by distance ASC** (nearest first)
- **Only active stations** returned
- **Max radius enforced** server-side (10km default MVP-1)
- **Distance** is integer in meters

---

### 1.3 Get Station by ID
...
| power_kw | number | No | Maximum power capacity in kW |
| connector_types | string[] | No | Supported connector types |
| chargers | array | No | List of available chargers |

**Charger Fields:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| id | string | Yes | Charger identifier (CHR-xxx) |
| type | string | Yes | Charger type (type2, ccs, chademo) |
| power_kw | number | Yes | Charger power in kW |
| status | string | Yes | Charger status (available, busy, offline) |

---

## 📊 ANALYTICS API (MINIMAL MVP-1)
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| type | string | Yes | Event type (MapViewed, StationOpened, NearbySearchExecuted) |
| user_id | string | No | User identifier (optional for MVP-1) |
| station_id | string | No | Station identifier (optional) |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| code | string | Yes | Error code (uppercase, descriptive) |
