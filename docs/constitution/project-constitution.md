...
business logic. Analytics data MUST remain separate from business state in the
`analytics` schema (JSONB columns). Only four PostgreSQL schemas are permitted:
`inventory`, `users`, `gis`, and `analytics`. No additional application schemas
MAY exist.
...
- **Analytics**: PostgreSQL (`analytics` schema, JSONB columns)
- **Identity**: Keycloak
...
2. **Clickstream events** (page.viewed, station.opened, search.performed,
   filter.applied, favorite_station.added, review.submitted) — published
    to RabbitMQ and persisted to the `analytics` schema.
...
- **gis** schema owns: OSM imports, roads, boundaries, derived spatial
   projections
- **analytics** schema owns: clickstream connection aggregates, raw events,
   and any other analytics data (all flexible via JSONB columns)
...
- No schema expansion without approval (only 4 schemas permitted: inventory, users, gis, analytics)
